 ...
 *
 * Google Apps Script ContentService redirects
 * the response. fetch follows the redirect.
 */
...
export default async function handler(
  ...
  if (!HMAC_SECRET) {
    return sendJson(
      ...
      500,
      {
        ok: false,
        error:
          'Submission security is not configured.',
      }
    );
  }
  ...
  try {
     ...
     * DEBUG FINGERPRINTS
     *
     * These do NOT expose the HMAC secret.
     */
    console.log(
      'TASK SECRET FINGERPRINT:',
      fingerprint(
        HMAC_SECRET.trim()
      )
    );
